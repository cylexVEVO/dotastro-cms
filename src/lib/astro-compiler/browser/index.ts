...
const instantiateWASM = async (
  ...
  if (WebAssembly.instantiateStreaming) {
    ...
    const fetchAndInstantiateTask = async () => {
      const wasmArrayBuffer = await fetch(wasmURL).then((res) =>
        res.arrayBuffer(),
      );
...
const startRunningService = async (wasmURL: string): Promise<Service> => {
  ...
  return {
    transform: (input, options) =>
      new Promise((resolve) =>
        resolve(service.transform(input, options || {})),
      ),
    convertToTSX: (input, options) =>
      new Promise((resolve) =>
        resolve(service.convertToTSX(input, options || {})),
      ).then((result: any) => ({
        ...result,
    ...
    parse: (input, options) =>
      new Promise((resolve) =>
        resolve(service.parse(input, options || {})),
